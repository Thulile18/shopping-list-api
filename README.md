...
│   └── server.ts           # Everything else: in-memory store, validation,
│                            # response helpers, route handlers, and the
│                            # HTTP server itself
├── package.json
├── tsconfig.json
└── README.md
...
In Postman, create requests for each endpoint above, pointing at `http://localhost:3000/items` (add `/:id` where needed).
For POST and PUT requests, set the body type to `raw` → `JSON` in Postman and include the fields shown above.
Try a few error cases too — e.g. POST with no `name`, or GET/DELETE with a made-up id — to confirm the 400/404 responses work.
Notes
Data is not persisted anywhere — it's just a JavaScript array inside `server.ts`, so it resets every time the server restarts. That's expected for this project.